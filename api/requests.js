...
export default async function handler(req, res) {
  ...
  const { gymId, clientId, id } = req.query;
  ...
  switch (req.method) {
    ...
    case 'PUT':
      try {
        if (!id) {
            return res.status(400).json({ message: 'Request ID is required for PUT' });
        }
        ...
        console.error(`API /api/requests [PUT] Error for id ${id}:`, e);
    ...
    case 'DELETE':
      try {
        if (!id) {
            return res.status(400).json({ message: 'Request ID is required for DELETE' });
        }
        if (!ObjectId.isValid(id)) {
          return res.status(400).json({ message: 'Invalid Request ID format' });
        }
        ...
        console.error(`API /api/requests [DELETE] Error for id ${id}:`, e);
